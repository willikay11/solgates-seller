...
export default function Dashboard() {
  ...
  const {
    ...
    hasNextPage,
    isFetchingNextPage,
  } = useProducts(user?.storeId, debouncedSearchQuery);
  const {
    mutate: logout,
  ...
  const shareProduct = async ({ message, title }: { message: string; title: string }) => {
    setIsSharing(true);
    try {
      const shareOptions = {
        title,
  ...
  useEffect(() => {
    ...
    if (isUpdateProductSuccess) {
      ...
      Toast.show({
        ...
        text2:
          (updateProductError as any)?.response?.data?.message ||
          (updateProductError as any)?.message ||
          'Please try again',
      });
    }
  ...
  return (
    <View style={[styles.scrollContainer, { marginTop: 40 }]}>
      ...
      <Modal
        ...
        setModalVisible={setWithdrawVisible}
        title="Withdraw Cash"
      >
        <View style={styles.withdrawContainer}>
          {/* <Text style={[styles.withdrawText, { marginBottom: 10 }]}>Enter the number you wish to receive the money on.</Text> */}
          <Text style={styles.withdrawText}>
            <Text style={styles.withdrawTextBold}>NOTE:</Text> Transaction cost of KES 20.00 will be
            charged.
          </Text>
          <View style={styles.withdrawTextContainer}>
            <Text style={styles.withdrawText}>
              <Text style={styles.withdrawTextBold}>Withdrawal Number:</Text> +{user?.phoneNumber}
            </Text>
          </View>
          <View style={styles.inputContainer}>
            <Input
              prefixComponent={<Icon name="hand-coin-line" size={14} color="#10B981" />}
              placeholder="Amount"
              value={amount}
          ...
          >
            <View style={styles.buttonContent}>
              ...
              <Text style={styles.buttonText}>
                {isWithdrawing ? 'Withdrawing...' : 'Withdraw Cash'}
              </Text>
            </View>
          </Button>
        </View>
        ...
        <View style={styles.headerContainer}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
            <Button
              ...
              style={[styles.iconButton, { height: 40, width: 40, backgroundColor: '#FEE2E2' }]}
            >
      ...
      <View style={styles.productContainer}>
        <Text style={styles.productHeaderText}>
          Your Stock ({getMetaFromPages(products)?.total ?? productsList.length} Products)
        </Text>
          ...
          <SwipeListView
            ...
            renderItem={({ item, index }: { item: Product; index: number }) => {
              ...
              return (
                ...
                >
                  <Image
                    source={{ uri: item.productImageUrls[0].url }}
                  ...
                  <View style={styles.productItemTextContainerLeft}>
                    <View style={styles.productItemTextContainer}>
                      ...
                      <Text style={styles.productItemTextDescription}>
                        {item.size.name} | {item.genders.map((gender) => gender.name).join(', ')}
                      </Text>
                      <Text style={styles.productItemText}>
                        KES {numeral(item.price).format('0,0.00')}
                      </Text>
                    </View>
