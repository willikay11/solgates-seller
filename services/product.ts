    ...
    try {
      ...
      formData.append(
        ...
        product.colours.length
          ? JSON.stringify(product.colours.map((colour: string) => ({ colour_id: colour })))
          : '',
      );
      formData.append(
        'genders',
        product.genders.length
          ? JSON.stringify(product.genders.map((gender: string) => ({ gender_id: gender })))
          : '',
      );

      formData.append(
        'product_image_urls',
        product.productUrls.length ? JSON.stringify(product.productUrls) : '',
      );
