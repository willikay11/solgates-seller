...
export default function ProductForm({ product }: ProductFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    if (product?.id && productUrls.length) {
      ...
      productUrls
        .filter((productUrl) => !currentProductUrls.includes(productUrl.url))
        .map((url) => {
          return uploadImage({
            uri: url.url,
            type: 'image/jpeg',
  ...
  return (
    <View style={styles.formContainer}>
      ...
      <ImagePicker
        ...
        selectedImages={productUrls.map((image) => image.url)}
      ...
      <CategoryList
        ...
        data={
          selectedCategory
            ? (categoryTypes
                ?.filter((categoryType) => categoryType.category.id === selectedCategory)
                .map((categoryType) => ({ id: categoryType.id, label: categoryType.name })) ?? [])
            : []
        }
